...
Credentials (AMO API key + secret) are resolved in this order:
  ...
  3. htcli config:  htcli config set amo-api-key / amo-api-secret
...
Get credentials at:
  https://addons.mozilla.org/en-US/developers/addon/api/key/`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		key := publishAPIKey
		if key == "" {
			key = firstEnv("AMO_API_KEY", "HTCLI_AMO_API_KEY")
		}
		if key == "" {
		...
		if secret == "" {
			secret = firstEnv("AMO_API_SECRET", "HTCLI_AMO_API_SECRET")
		}
		if secret == "" {
		...
		if key == "" || secret == "" {
			return fmt.Errorf(
				"AMO API credentials required: pass --api-key/--api-secret, set " +
					"AMO_API_KEY/AMO_API_SECRET, or run `htcli config set amo-api-key/amo-api-secret`",
			)
		}
